...
$(document).ready(function () {
    /** --------------------------------------------------------------------------------------------------
     *  [messages] - chat composer bindings (moved from messages.js)
     * -------------------------------------------------------------------------------------------------*/
    $(document).off('click.messages', '#send_message_btn').on('click.messages', '#send_message_btn', function (e) {
        e.preventDefault();
    ...
    $(document).off('click.messages', '#file_upload_btn').on('click.messages', '#file_upload_btn', function (e) {
        e.preventDefault();
    ...
    $(document).off('click.messages', '#emoji_btn').on('click.messages', '#emoji_btn', function (e) {
        e.preventDefault();
        const picker = $('#emoji_picker');
        picker.toggle();
        if (picker.is(':visible')) {
            if (typeof NX !== 'undefined' && typeof NX.initializeEmojiTabs === 'function') {
                setTimeout(function(){
                    NX.initializeEmojiTabs();
                    if (typeof NX.populateEmojiGrid === 'function') {
    ...
    // Emoji tabs: delegate click to filter grid
    $(document).off('click.messages-emoji-tabs', '#emoji_picker .emoji-tab')
        .on('click.messages-emoji-tabs', '#emoji_picker .emoji-tab', function (e) {
            e.preventDefault();
            // determine category
            var category = $(this).attr('data-category') || $(this).data('category') || $(this).attr('data-cat') || 'smileys';
            // toggle active state
    ...
    $(document).off('change.messages', '#channel_dropdown').on('change.messages', '#channel_dropdown', function () {
        const val = $(this).val();
        if (typeof NX !== 'undefined' && typeof NX.updateChannelSelector === 'function') {
            NX.updateChannelSelector(val);
        }
    });
...
/**--------------------------------------------------------------------------------------
 * [TASK CHECKLIST - DRAG & DROP]
 * @description: drag and drop checklist items
 * -------------------------------------------------------------------------------------*/
function taskChecklistDragDropX() {
    var container = document.getElementById('card-checklists-container');
    if (container) {
        let itemDraggable = dragula([container], {
            moves: function (el, source, handle, sibling) {
                // Only allow elements with class 'checklist-item' to be dragged
                return el.classList.contains('checklist-item');
            }
        });

        itemDraggable.on('drag', function (element) {
            element.classList.add('is-moving');
        });

        itemDraggable.on('dragend', function (element) {
            element.classList.remove('is-moving');

            // Update via AJAX (ensure correct selector)
