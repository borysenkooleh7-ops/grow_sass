...
/**
 * set a checkbox as 'checked' based on a boolean or truthy value
 * @param mixed $value value normally coming from database (boolean, int, string)
 * @return string
 */
function runtimeChecked($value = false) {
    // Check if value is truthy (true, 1, 'yes', 'on', 'enabled', etc.)
    $truthy_values = [
        true,
        1,
        '1',
    ...
    if (in_array($value, $truthy_values, true) || $value === true || $value == 1) {
        return 'checked';
    }
    return '';
}
...
/**
 * Count WhatsApp tickets for a task
 * @param int $taskId
 * @return int
 */
if (!function_exists('whatsappTicketsCountForTask')) {
    function whatsappTicketsCountForTask($taskId) {
        if (!$taskId) {
            return 0;
        }

        return \App\Models\WhatsappTicket::where('whatsappticket_taskid', $taskId)->count();
    }
}
