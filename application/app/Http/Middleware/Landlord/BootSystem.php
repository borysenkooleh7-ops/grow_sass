...
class BootSystem {
    ...
    public function handle($request, Closure $next) {
        ...
        //system sanity check
        $sanityCheckResult = $this->sanityCheck();
    ...
    public function customerDefaults() {
        ...
        //if defaults not found, set empty config
        if (!$defaults) {
            config(['customer_defaults' => []]);
            return;
        }
    ...
    public function systemSettings() {
        ...
        //if settings not found, set defaults and return
        if (!$settings) {
    ...
    public function setBackEndTheme() {
        ...
        //if settings not found, use default theme
        if (!$this->settings) {
            config([
                'theme.backend.selected_theme_css' => 'public/themes/default/css/style.css?v=1',
                'theme.backend.selected_name' => 'default',
            ]);
            return;
        }
        ...
        if (!in_array($this->settings->settings_theme_name, $directories)) {
            //fallback to default theme instead of aborting
            $this->settings->settings_theme_name = 'default';
        }
