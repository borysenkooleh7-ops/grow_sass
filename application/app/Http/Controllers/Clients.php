...
class Clients extends Controller {
    ...
    public function showDynamic($id) {
        ...
        case 'feedback':
            ...
            break;
        case 'clientai':
            $page['dynamic_url'] = url('clients/' . $client->client_id . '/client-clientai');
            break;
        default:
            $page['dynamic_url'] = url("timeline/client?request_source=client&source=ext&timelineclient_id=$id&page=1");
            break;
    ...
    /**
     * To show Expectativas of selected client
     * @param mixed $id
     * @return DetailsResponse
     */
    public function expectativas($id) {
        ...
        //response
        return new ExpectativasResponse($payload);
    }
    /**
     * To show feedback detail information of Client
     * @param mixed $id
     * @return \App\Http\Responses\Clients\FeedbackResponse
     */
    public function feedback($id) {
        ...
        $feedbacks = $this->feedbackrepo->getFeedbackSummariesForClient((int) $id);
        //reponse payload
        $payload = [
            'page' => $this->pageSettings('client', $client),
            'client' => $client,
        ...
        //response
        return new FeedbackResponse($payload);
    }
    ...
    public function clientai($id) {
        ...
        //response
        return new ClientAIResponse($payload);
    }
    ...
    public function generateAIFeedbackAnalysis($clientId)
    {
        try {
            ...
            $messages = [
                [
                    'role' => 'system',
                    'content' => 'You are an expert business analyst AI. Analyze client feedback and provide actionable insights in a clear, professional format.'
                ],
                [
                    'role' => 'user',
                    'content' => $prompt
    ...
    public function generateAIExpectationsAnalysis($clientId)
    {
        try {
            ...
            $messages = [
                [
                    'role' => 'system',
                    'content' => 'You are an expert business analyst AI. Analyze client expectations and provide actionable insights in a clear, professional format.'
                ],
                [
                    'role' => 'user',
                    'content' => $prompt
    ...
    public function generateAIProjectsAnalysis($clientId)
    {
        try {
            ...
            $messages = [
                [
                    'role' => 'system',
                    'content' => 'You are an expert business analyst AI. Analyze client projects and provide actionable insights in a clear, professional format.'
                ],
                [
                    'role' => 'user',
                    'content' => $prompt
    ...
    public function generateAICommentsAnalysis($clientId)
    {
        try {
            ...
            $messages = [
                [
                    'role' => 'system',
                    'content' => 'You are an expert business analyst AI. Analyze client comments and provide actionable insights in a clear, professional format.'
                ],
                [
                    'role' => 'user',
                    'content' => $prompt
    ...
    /**
     * Display WhatsApp tab for client
     *
     * @param int $id client id
     * @return \Illuminate\Http\Response
     */
    public function showWhatsapp($id) {

        //get the client with WhatsApp relationships
        $client = \App\Models\Client::with([
            'whatsappContact',
            'whatsappTickets' => function($query) {
                $query->with(['assignedAgent', 'task'])
        ...
        //page settings
        $page = $this->pageSettings('client', $client);
        $page['section'] = 'whatsapp';

        //render view
        $html = view('pages.client.components.tabs.whatsapp', compact('client', 'page'))->render();
