            ...
            <label for="toggle_task_status_{{ $task->task_id }}">
                <a class="show-modal-button reset-card-modal-form js-ajax-ux-request" href="javascript:void(0)"
                    ...
                    data-loading-target="main-top-nav-bar"><span class="x-strike-through"
                        ...
                        @if($task->whatsappTicket)
                            <a href="javascript:void(0);"
                               onclick="openWhatsappConversation({{ $task->task_id }})"
                               class="badge badge-success ml-1"
                               title="Open WhatsApp conversation"
                               style="background: #25D366;">
                                <i class="fab fa-whatsapp"></i>
                                @if($task->whatsappTicket->whatsappticket_unread_count > 0)
                                    {{ $task->whatsappTicket->whatsappticket_unread_count }}
                                @endif
                            </a>
                        @endif
