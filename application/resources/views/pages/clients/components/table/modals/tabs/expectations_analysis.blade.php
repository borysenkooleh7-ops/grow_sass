<div class="expectations-analysis">
    <div class="card mb-3">
        <div class="card-header bg-info text-white">
            <h6><i class="fas fa-bullseye"></i> Expectation Progress</h6>
        </div>
        <div class="card-body">
            @if(!$expectationData['has_recent_progress'])
                <div class="alert alert-warning">No progress on objectives in the previous 3 months.</div>
            @else
                <div class="alert alert-success">Recent progress on objectives detected.</div>
            @endif
        </div>
    </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="ai-prompt-area d-none">
                    <textarea class="form-control" rows="8" readonly>{{ json_encode($expectationData) }}</textarea>
                    <small class="text-muted">This prompt will be sent to OpenAI for analysis</small>
                </div>
                <div class="mt-3">
                    <button class="btn btn-sm btn-primary ai-analysis-btn" data-toggle="tooltip" data-placement="top" title="Generate AI-powered analysis" onclick="generateAIAnalysis('expectations', {{ $client->client_id }})">
                        <i class="fas fa-magic"></i> Generate AI Analysis
                    </button>
                </div>
                <div id="ai-response-expectations" class="mt-3" style="display: none;">
