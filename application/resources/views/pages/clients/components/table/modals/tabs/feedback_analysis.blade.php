@php
    $feedbackList = $latestFeedbacks ?? [];
@endphp
<div class="feedback-analysis">
    <div class="card mb-3">
        <div class="card-header bg-primary text-white">
            <h6><i class="fas fa-comments"></i> Latest Feedbacks</h6>
        </div>
        <div class="card-body">
            @if(count($feedbackList) === 0)
                <div class="alert alert-warning">No feedback found for this client.</div>
            @else
                <table class="table table-sm table-bordered mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $fb->feedback_created }}</td>
                            <td>{{ $fb->total_marks }}</td>
                            <td>{{ $fb->comment }}</td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="ai-prompt-area d-none">
                    <textarea class="form-control" rows="8" readonly>{{ json_encode($feedbackData) }}</textarea>
                    <small class="text-muted">This prompt will be sent to OpenAI for analysis</small>
                </div>
                <div class="mt-3">
                    <button class="btn btn-sm btn-primary ai-analysis-btn" data-toggle="tooltip" data-placement="top" title="Generate AI-powered analysis" onclick="generateAIAnalysis('feedback', {{ $client->client_id }})">
                        <i class="fas fa-magic"></i> Generate AI Analysis
                    </button>
                </div>
                <div id="ai-response-feedback" class="mt-3" style="display: none;">
