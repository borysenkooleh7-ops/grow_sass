<div class="comments-analysis">
    <div class="card mb-3">
        <div class="card-header bg-info text-white">
            <h6><i class="fas fa-question-circle"></i> Unanswered Client Comments</h6>
        </div>
        <div class="card-body">
            @if(count($commentsData) > 0)
                <div class="alert alert-info">Unanswered client comments:
                    <ul>
                        @foreach($commentsData as $comment)
                            <li>{{ $comment->comment }} <span class="text-muted">({{ $comment->feedback_created }})</span></li>
                        @endforeach
                    </ul>
                </div>
            @else
                <div class="alert alert-success">No unanswered client comments.</div>
            @endif
        </div>
    </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="ai-prompt-area d-none">
                    <textarea class="form-control" rows="8" readonly>{{ json_encode($commentsData) }}</textarea>
                    <small class="text-muted">This prompt will be sent to OpenAI for analysis</small>
                </div>
                <div class="mt-3">
                    <button class="btn btn-sm btn-primary ai-analysis-btn" data-toggle="tooltip" data-placement="top" title="Generate AI-powered analysis" onclick="generateAIAnalysis('comments', {{ $client->client_id }})">
                        <i class="fas fa-magic"></i> Generate AI Analysis
                    </button>
                </div>
                <div id="ai-response-comments" class="mt-3" style="display: none;">
