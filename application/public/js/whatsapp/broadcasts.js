...
/**
 * WHATSAPP BROADCASTS - MODAL POSTRUN FUNCTION
 * Called after modal is loaded
 */
function NXWhatsappBroadcasts() {
    // Form validation
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
            broadcast_message: "required",
            broadcast_connection_id: "required"
        },
    ...
    if (!$('.select2-basic').hasClass('select2-hidden-accessible')) {
        $('.select2-basic').select2();
    }
    if (!$('.select2-multiple').hasClass('select2-hidden-accessible')) {
        $('.select2-multiple').select2();
    }
    if (!$('.select2-multiple-tags').hasClass('select2-hidden-accessible')) {
